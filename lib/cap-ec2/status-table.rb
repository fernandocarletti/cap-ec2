module CapEC2
  class StatusTable
    ...
    def instance_to_row(instance, index)
      [
        sprintf("%02d:", index),
        green(instance.tags.select{|tag| tag.key == "Name"}.first.value || ''),
        red(instance.id),
        cyan(instance.instance_type),
        bold(blue(CapEC2::Utils.contact_point(instance))),
        magenta(instance.placement.availability_zone),
        yellow(instance.tags.select{|tag| tag.key == roles_tag}.first.value),
        yellow(instance.tags.select{|tag| tag.key == stages_tag}.first.value)
      ]
    end
